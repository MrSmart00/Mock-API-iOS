...
Each operation lives under the `MockAPI` namespace and within an optional tag: `MockAPI(.tagName).operationId`. If an operation doesn't have an operationId one will be generated from the path and method.
...
let getUserRequest = MockAPI.User.GetUser.Request(id: 123)
...
Each [Request](#request) also has a `makeRequest` convenience function that uses `MockAPI.shared`.
...
There are some options to control how invalid JSON is handled when decoding and these are available as static properties on `MockAPI`:
